...
-High genetic variation: 2 possible alleles
-Mutation in vector is given a value of 1e-4 for all populations
-Migration among populations increases with time
-The files for this simulation are saved with LGV at the end, which stands for low genetic variation. This is done to set
apart this simulation from another with same parameters but high genetic variation"""
...
my_model.newSetup('highT',preset='vector-borne', num_loci=5,possible_alleles="DW",contact_rate_host_vector=1e3, mutate_in_host=1e-4)
my_model.newSetup('mediumT',preset='vector-borne', num_loci=5,possible_alleles="DW",contact_rate_host_vector=1e1,mutate_in_host=1e-4)
my_model.newSetup('lowT',preset='vector-borne', num_loci=5,possible_alleles="DW",contact_rate_host_vector=1e-1, mutate_in_host=1e-4)
...
my_model.addPathogensToHosts( 'population_mediumT',{'DWDWD':10})
...
data = my_model.saveToDataFrame('VarMigr_TransmLGV.csv')
    # Save the model results to a table.
graph = my_model.compartmentPlot('VarMigr_Transm_HostsLGV.png', data)
    # Plot the number of susceptible and infected hosts in the model over time.
graph2 = my_model.compositionPlot('VarMigr_Transm_GenotypesLGV.png',data)
    #Plot the track genotypes across time
graph3= my_model.populationsPlot('VarMigr_Transm_PopulationLGV',data)
