...
-High transmission (contact_rate_host_vector) for all popoulations = 1e2
-Mutation in vector is given a value of 1e-4 for all populations
-Migration among populations increases with time
-The files for this simulation are saved with HT at the end, which stands for high transmission rate. This is done to set
apart this simulation from another with same parameters but low transmission rate"""
...
my_model.newSetup('highGV_LT',preset='vector-borne', num_loci=5,possible_alleles="DRSTW",contact_rate_host_vector=1e2, mutate_in_host=1e-4)
my_model.newSetup('mediumGV_LT',preset='vector-borne', num_loci=5,possible_alleles="BCY",contact_rate_host_vector=1e2,mutate_in_host=1e-4)
my_model.newSetup('lowGV_LT',preset='vector-borne', num_loci=5,possible_alleles="A",contact_rate_host_vector=1e2, mutate_in_host=1e-4)
...
graph = my_model.compartmentPlot('VarMigrStandVar_HostsHT.png', data)
    # Plot the number of susceptible and infected hosts in the model over time.
graph2 = my_model.compositionPlot('VarMigr_StanVar_GenotypesHT.png',data)
    #Plot the track genotypes across time
graph3= my_model.populationsPlot('VarMigr_StanVar_PopulationHT',data)
