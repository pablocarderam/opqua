...
def compartmentPlot(file_name, data, populations=[], hosts=True, vectors=False,
    ...
    if comp.shape[1] > 0:
        ...
        if stacked:
            ax.stackplot( comp['Time'], comp.drop(columns='Time').transpose(), labels=comp.columns, colors=palette )
        else:
...
def compositionPlot(file_name, data, populations=[], type='Pathogens', hosts=True, vectors=False,
