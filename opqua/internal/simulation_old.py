...
class Simulation(object):
    ...
    RECOVER_HOST = 8
    RECOVER_VECTOR = 9
    MUTATE_ESTABLISH_HOST = 10
    MUTATE_ESTABLISH_VECTOR = 11
    FIXATE_HOST = 12
    FIXATE_VECTOR = 13
    RECOMBINE_HOST = 14
    RECOMBINE_VECTOR = 15
    ...
    EVENT_IDS = { # must match above
        ...
        MUTATE_ESTABLISH_HOST:'MUTATE_ESTABLISH_HOST',
        MUTATE_ESTABLISH_VECTOR:'MUTATE_ESTABLISH_VECTOR',
    ...
    def __init__(self, model):
        ...
        self.evt_IDs = [
            ...
            self.MUTATE_ESTABLISH_HOST, self.MUTATE_ESTABLISH_VECTOR,
        ...
        if ( np.sum([
            ...
            self.evt_possible[self.MUTATE_ESTABLISH_VECTOR] = 0
            self.evt_possible[self.FIXATE_VECTOR] = 0
        ...
        if ( np.sum([
            ...
            self.evt_possible[self.MUTATE_ESTABLISH_HOST] = 0
        if ( np.sum([
    ...
    def getRates(self,population_ids):
        ...
        for i,id in enumerate(population_ids):
            ...
            if self.evt_possible[self.MUTATE_ESTABLISH_HOST]:
                rates[self.MUTATE_ESTABLISH_HOST,i] = (
                    self.model.populations[id].mutate_in_host
                    * self.model.populations[id].coefficients_hosts[
                        :, self.model.populations[id].MUTATE_ESTABLISH
                        ].sum()
                    )

            if self.evt_possible[self.MUTATE_ESTABLISH_VECTOR]:
                rates[self.MUTATE_ESTABLISH_VECTOR,i] = (
            ...
            if self.evt_possible[self.FIXATE_HOST]:
                rates[self.FIXATE_HOST,i] = (
                    self.model.populations[id].coefficients_hosts[
                        :, self.model.populations[id].FIXATION
                        ].sum()
                    )

            if self.evt_possible[self.FIXATE_VECTOR]:
                rates[self.FIXATE_VECTOR,i] = (
                    self.model.populations[id].coefficients_vectors[
                        :, self.model.populations[id].FIXATION
                        ].sum()
                    )
    ...
    def doAction(self,act,pop,rand):
        ...
        elif act == self.MUTATE_ESTABLISH_HOST:
            pop.mutateHost(rand)
            changed = True

        elif act == self.MUTATE_ESTABLISH_VECTOR:
            pop.mutateVector(rand)
            changed = True

        elif act == self.FIXATE_HOST:
            pop.fixateHost(rand)
            changed = True

        elif act == self.FIXATE_VECTOR:
            pop.fixateVector(rand)
            changed = True
